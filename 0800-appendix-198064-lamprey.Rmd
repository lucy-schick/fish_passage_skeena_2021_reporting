...
There are 4 other modelled crossings located upstream of PSCIS crossing `r as.character(my_site)` on this tributary to Lamprey Creek; the first one being approximately 2.25km upstream. PSCIS crossing `r as.character(my_site)` was ranked as a moderate
...
`r fpr_my_habitat_paragraph(loc = 'ds')` Abundant undercut banks were present, but few deep pools. Water flowed beneath the substrate approximately 5m from the confluence with Lamprey creek. The
...
`r fpr::fpr_my_priority_info(col_pull = priority)` priority for proceeding to design for replacement. A fish observation was confirmed at the time of survey at the crossing. It is recommended that future fish sampling be conducted upstream of the culvert to determine if fish are using this system for spawning and/or rearing.  
