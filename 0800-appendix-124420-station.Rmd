...
PSCIS crossing `r as.character(my_site)` is located on `r fpr::fpr_my_pscis_info()` in Hazelton, BC. Station Creek is also known locally as Mission Creek.  PSCIS crossing `r as.character(my_site)` is located on `r fpr_my_pscis_info(col_pull = road_name)`, approximately `r round(fpr::fpr_my_bcfishpass(col_pull = downstream_route_measure)*0.001, 1)`km upstream from the confluence with the Bulkley River. The crossing is the responsibility of the Ministry of Transportation and Infrastructure.

<br>

Approximately 1200m upstream of PSCIS crossing `r as.character(my_site)`, Waterfall Creek joins Station Creek. Waterfall Creek is mapped incorrectly in the BC freshwater atlas which documents it as a small drainage originating on the north side of Highway 16. In reality, the stream flows from an area south of town, through New Hazelton in a northern direction, crossing under numerous road crossings and two CN railway crossings before joining with Station Creek (Figure \@ref(fig:fig-waterfall)). There is a waterfall that blocks upstream migration for all life stages and species, located approximately 1200m upstream of the easternmost Waterfall Creek/Highway 16 crossing (PSCIS 198116), on Waterfall Creek. 

<br>

PSCIS crossings `r as.character(my_site2)` and `r as.character(my_site3)` are located on Waterfall Creek. Crossing `r as.character(my_site2)` is located on `r fpr_my_pscis_info(site = my_site2, col_pull = road_name)`, approximately `r round(fpr::fpr_my_bcfishpass(site = my_site2, col_pull = downstream_route_measure)*0.001, 1)` km upstream from the confluence with Station Creek. The crossing is the responsibility of the Town of New Hazelton. Approximately, 120m upstream, crossing `r as.character(my_site3)` is located on `r fpr_my_pscis_info(site = my_site3, col_pull = road_name)`, and is the responsibility of Ministry of Transportation and Infrastructure.
...
my_caption = 'Map of Station Creek and Waterfall Creek. Pink line indicates the corrected streamline for Waterfall Creek.'

knitr::include_graphics('fig/map_station_waterfall.jpeg')
...
PSCIS crossing `r as.character(my_site)` located on Highway 16 and Station Creek was ranked as a high priority for follow up by @wilson_rabnett2007FishPassage, @mccarthy_fernando20152015Inventory,  @irvine2018AnalysisPriority and @mazany-wright_etal2021BulkleyRiver because of known high value spawning and rearing habitat upstream of the crossing. The crossing was reassessed in 2021 and a habitat confirmation was conducted, which also included crossings `r as.character(my_site2)` and `r as.character(my_site3)`. A summary of habitat modelling outputs is presented in Table \@ref(tab:tab-culvert-bcfp-124420). A map of the watershed is provided in map attachment [`r fpr::fpr_my_bcfishpass(col_pull = dbm_mof_50k_grid)`](`r fpr::fpr_my_mapsheet()`).

<br>

Three additional crossings further upstream on Waterfall Creek—PSCIS 198117, 198116, and 198115—were assessed in 2022 by @irvine_etal2023BulkleyWatershed (Figure \@ref(fig:fig-waterfall)). In 2024, the stream was revisited and surveyed from the eastern Waterfall Creek/Highway 16 crossing (PSCIS 198116) upstream to the waterfall. Detailed assessments of these surveys, along with additional information regarding the Waterfall/Station system, are provided in [Bulkley Watershed Fish Passage Restoration Planning 2022](https://www.newgraphenvironment.com/fish_passage_bulkley_2022_reporting/).
...
PSCIS crossing 124421 on Waterfall Creek was un-embedded, non-backwatered and ranked as a `r fpr::fpr_my_pscis_info(site = my_site2, col_pull = barrier_result) %>% stringr::str_to_lower()` to upstream fish passage according to the provincial protocol [@moe2011Fieldassessment] (Table \@ref(tab:tab-culvert-124421)). 

<br>

PSCIS crossing 124422 on Waterfall Creek was un-embedded, 100% backwatered and ranked as a `r fpr::fpr_my_pscis_info(site = my_site3, col_pull = barrier_result) %>% stringr::str_to_lower()` to upstream fish passage according to the provincial protocol [@moe2011Fieldassessment] (Table \@ref(tab:tab-culvert-124422)). 
...
In 2021, an aerial survey was conducted with a remotely piloted aircraft upstream of where `r fpr::fpr_my_pscis_info()` crosses `r fpr_my_pscis_info(col_pull = road_name)` (PSCIS `r as.character(my_site)`) and the resulting images were stitched into an orthomosaic and 3-dimensional model `r if(gitbook_on){knitr::asis_output(" presented in Figures \\@ref(fig:ortho-caption-124420) - \\@ref(fig:model-caption-124420).")} else knitr::asis_output("presented [here](https://www.mapsmadeeasy.com/maps/public/8f568e62bb7d47568e1f0a365022ef59) and [here](https://www.mapsmadeeasy.com/maps/public_3D/8f568e62bb7d47568e1f0a365022ef59).")`

<br>

In 2024, a second aerial survey was conducted where Waterfall Creek crosses Highway 16 (PSCIS `r as.character(my_site3)`) and the CN railway (PSCIS 124423), with an orthomosaic available to view and download `r if(gitbook_on){knitr::asis_output("in Figure \\@ref(fig:uav-ortho-124423)")}else(knitr::asis_output("[here](https://viewer.a11s.one/?cog=https://imagery-uav-bc.s3.amazonaws.com/skeena/bulkley/2024/124423_waterfall_cnrail/odm_orthophoto/odm_orthophoto.tif)"))`.

<br>

```{r ortho-124420, eval = identical(gitbook_on, TRUE)}
...
```{r ortho-caption-124420, fig.cap= my_caption, out.width = "0.01%", eval = identical(gitbook_on, TRUE)}
...
```{r model-124420, eval = identical(gitbook_on, TRUE)}
...
```{r model-caption-124420, fig.cap= my_caption, out.width = "0.01%", eval = identical(gitbook_on, TRUE)}
my_photo = 'fig/pixel.png'
...
```{r uav-ortho-124423-prep, eval = gitbook_on}
viewer_url <- '<iframe src="https://viewer.a11s.one/?cog=https://imagery-uav-bc.s3.amazonaws.com/skeena/bulkley/2024/124423_waterfall_cnrail/odm_orthophoto/odm_orthophoto.tif" scrolling="no" title="UAV Viewer" width="100%" height="600" frameBorder="0"></iframe>'

knitr::asis_output(viewer_url)
```

```{r uav-ortho-124423, out.width = "0.01%", eval = gitbook_on, fig.cap= my_caption}
my_photo = 'fig/pixel.png'

my_caption = paste0('Orthomosaic of where Waterfall Creek crosses Highway 16 (PSCIS ', my_site3, '), and the CN railway (PSCIS 124423).')
...
As of 2024, remediation efforts to replace the 11th Avenue crossing on Waterfall Creek (PSCIS `r as.character(my_site2)`) are tentatively planned for summer 2025. The project is being led by Gitksan Watershed Authorities in collaboration with SERNbc, Skeena Fisheries Commission, Fisheries and Oceans Canada, Gitsxan House representatives, the Chicago Creek Community Environmental Enhancement Society, and the District of New Hazelton. As part of the 2024 work, SERNbc funded the development of an engineering design for a bridge to replace the existing open-bottom culverts. The design was prepared by Pacific North Coast Consulting Services. Additional information is provided in the [2024 report](https://www.newgraphenvironment.com/fish_passage_skeena_2024_reporting/).
