...
hab_loc <- habitat_confirmations %>%
  ...
  ## We need to remove the following sites because they were re sampling events to determining surveyor efficiency
  dplyr::filter(!(alias_local_name %in% c('123377_ds_ef4', '123377_ds_ef5', '123377_ds_ef6')))
...
hab_fish_indiv_prep <- habitat_confirmations %>%
  ...
  dplyr::filter(!is.na(site_number)) %>%
  ## We need to remove the following sites because they were re sampling events to determining surveyor efficiency
  dplyr::filter(!(local_name %in% c('123377_ds_ef4', '123377_ds_ef5', '123377_ds_ef6'))) |>
