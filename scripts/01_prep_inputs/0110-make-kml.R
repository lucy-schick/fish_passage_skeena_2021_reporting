...
dat1 <- fpr::fpr_import_pscis(workbook_name = 'pscis_phase1.xlsm')
...
make_pscis_kml_prep <- function(workbook){
  fpr::fpr_import_pscis(workbook_name = workbook)  %>%
    make_kml_col()  %>%
...
coords <- dat %>% select(easting, northing)
...
plotKML::kml_open(paste0("data/outgoing/",bname, "_PASSAGE_assessments.kml"))
plotKML::kml_layer(dat, shape = dat$shape, colour = dat$color, labels = dat$label, html.table = dat$html_tbl, z.scale = 2, LabelScale = 1, size = 2)
plotKML::kml_close(paste0("data/outgoing/",bname, "_PASSAGE_assessments", format(Sys.time(),"_%Y%m%d_%H%M.kml")))
