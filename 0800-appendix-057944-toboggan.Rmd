...
Works to remediate fish passage issues at PSCIS stream crossing `r as.character(my_site)` were conducted in 2017 with backwatering completed through installation of boulders downstream of the crossing.  Comparison of historic PSCIS photos and data between 2021 and 2012 indicate that the historical recorded outlet drop of 90cm has been reduced to 0cm as a result of the works.  Sandra Devcic, DFO Restoration Engineer (pers. comm.) has noted that developing a design to replace the crossing with an open bottom structure presents challenges due to the stream crossing location at the bottom of a hill and on a corner of the highway. 
...
PSCIS stream crossing `r as.character(my_site)` was ranked as a high priority for follow up by @wilson_rabnett2007FishPassage,  @irvine2021BulkleyRiver and @mazany-wright_etal2021BulkleyRiver because of significant amounts of habitat modelled as upstream of the crossing.  A summary of habitat modelling outputs is presented in Table \@ref(tab:tab-culvert-bcfp-57944). A map of the watershed is provided in map attachment [`r fpr::fpr_my_bcfishpass(col_pull = dbm_mof_50k_grid)`](`r fpr::fpr_my_mapsheet()`).  
...
At the time of the survey, PSCIS crossing `r as.character(my_site)` was un-embedded, non-backwatered and ranked as a `r fpr::fpr_my_pscis_info(col_pull = barrier_result) %>% stringr::str_to_lower()` barrier to upstream fish passage according to the provincial protocol [@moe2011Fieldassessment] (Table \@ref(tab:tab-culvert-57944)). 
...
The stream was also surveyed at a second location upstream of the hatchery adjacent to the Owen Road bridge for `r fpr::fpr_my_priority_info(loc = 'us2', col_pull = 'length_surveyed')`m `r if(gitbook_on){knitr::asis_output("(Figures \\@ref(fig:photo-57944-05) - \\@ref(fig:photo-57944-06))")}else(knitr::asis_output("(Figure \\@ref(fig:photo-57944-d03))"))`. Total cover amount was rated as `r fpr::fpr_my_habitat_cover(loc = 'us2', cover_type = 'total cover')` with  `r fpr::fpr_my_habitat_cover(loc = 'us2', cover_type = 'dominant')` dominant. Cover was also present as `r fpr::fpr_my_habitat_cover(loc = 'us2', cover_type = 'sub-dominant')`. The average channel width was `r fpr::fpr_my_habitat_info(loc = 'us2', col_pull = 'avg_channel_width_m')`m, the average wetted width was `r fpr::fpr_my_habitat_info(loc = 'us2', col_pull = 'avg_wetted_width_m')`m and the average gradient was `r fpr::fpr_my_habitat_info(loc = 'us2', col_pull = 'average_gradient_percent')`%. The dominant substrate was `r fpr::fpr_my_habitat_info(loc = 'us2', col_pull = "bed_material_dominant")` with `r fpr::fpr_my_habitat_info(loc = 'us2', col_pull = "bed_material_subdominant")` subdominant.  The habitat was rated as `r fpr::fpr_my_priority_info(loc = 'us2')` value for salmon rearing and spawning. 
...
There was `r fpr::fpr_my_priority_info(col_pull= upstream_habitat_length_km)`km of habitat modelled upstream of crossing `r as.character(my_site)` rated as `r fpr::fpr_my_priority_info(sit = my_site, loc = 'us', col_pull = hab_value)` value for salmonid rearing and spawning. Crossing `r as.character(my_site)` was ranked as a `r fpr::fpr_my_priority_info(col_pull = priority)` priority for proceeding to design for replacement. Toboggan Creek represents an extremely important system for the Bulkley River system in terms of fisheries values as well as it's function for outreach wihtin the community as well as it's contribution to stock enhancement and assessment. Development of a plan to replace the crossing with a bridge may be considered as a prudent step to mitigating risks of a blow out of the backwatering works currently in place downstream.  
